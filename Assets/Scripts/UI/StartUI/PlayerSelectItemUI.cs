...
/// <summary>
/// 플레이어 선택 아이템 UI
/// 플레이어를 구입하거나 선택할 수 있는 UI
/// </summary>
public class PlayerSelectItemUI : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    #region 이벤트
    public event Action<PlayerData> OnPointerEntered;
    public event Action<PlayerData> OnPointerExited;
    public event Action<PlayerData> OnPointerClicked;
    #endregion
    ...
    {
        //이벤트 호출
        OnPointerEntered?.Invoke(PlayerData);
    }

    private void HandleOnPointerExited()
    {
        //이벤트 호출
        OnPointerExited?.Invoke(PlayerData);
    }

    private void HandlePointerClicked()
    {
        //이벤트 호출
        OnPointerClicked?.Invoke(PlayerData);
    ...
    #region 초기화
    public void Init(PlayerData playerData, bool isUnlocked, bool isSelected)
    {
        //데이터 설정
